...
+
IMPORTANT: Ensure Cluster set is set to `global`, otherwise you will have issues in the next steps.
+
...
. Select *Path* from the dropdown (if you don't see anything in the dropdown make sure you created new opeions in the previous steps):
+
[source,sh,subs="attributes",role=execute]
----
skupper-demo
