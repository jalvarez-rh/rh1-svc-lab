...
. Under *Sync Policy* uncheck *Automaticaly sync when cluster state changes* let's leave these settings as default.
+
...
[NOTE]
====
*equals any off* defaults to "In" meaning that the application will be deployed to the global cluster set with any label value of aws-us or local-cluster.
====
+

IMPORTANT: We will use these applications in other modules of the lab hence why we have you deploy it twice in two different different clusters.
