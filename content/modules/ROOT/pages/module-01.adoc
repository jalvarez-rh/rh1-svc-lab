...
. Next, select the release image *OpenShift 4.20.10*. 
...
While we wait for the cluster to deploy, lets tag the existing *aws-us* cluster with a location tag, we will leverage this tag once we deploy an application.

[start=1]
. On the *Fleet Managment* cluster view, locate the *aws-us* cluster 
. Locate the elipses next to creatation date and select *Edit Labels* 
...
[start=15]
. While this application deploys, Repeat the steps to deploy the application to the *hcp-emea* cluster. Make sure the application name is set to *skupper-patient-demo-emea*
...
image::03-app-placement-emea.png[link=self, window=blank, width=100%, ACM App Placement EMEA]

[start=16]
. Navigate back to the application view, find the filter and select *Application Set*
. Find the *skupper-patient-demo-us* app 
