...
* xref:module-01.adoc[Module 1: Operations & Security] - Multi-environment cluster and workload management
* xref:module-02.adoc[Module 2: Assurance] - Cluster conformance, compliance, and security insights
* xref:module-03.adoc[Module 3: Technology & Supply Chain] - Secure supply chain and SLSA framework
* xref:module-04.adoc[Module 4: Data & AI] - Data pipelines and AI compute capabilities

== Getting Started

Before proceeding with the lab modules, you'll need to configure kubectl contexts for the clusters and install the roxctl CLI tool.
...
==== roxctl CLI and RHACS setup

Before proceeding with the lab modules, you'll need to install the roxctl CLI tool and verify connectivity to RHACS.

*Procedure*

[start=1]
. Add the RHACS environment variables to your ~/.bashrc file:
...
. Clone the repository and run the ACS setup script to install `roxctl` and configure RHACS access:

[source,sh,role=execute]
----
cd ~ && git clone https://github.com/jalvarez-rh/rh1-svc-lab.git || (cd ~/rh1-svc-lab && git pull origin main)
cd ~/rh1-svc-lab/setup-files
./acs-setup.sh
----
...
The Red Hat Advanced Cluster Management for Kubernetes (RHACM) Console is now integrated with OpenShift Console. To access it, select *All Clusters* from the dropdown menu once you log in.
