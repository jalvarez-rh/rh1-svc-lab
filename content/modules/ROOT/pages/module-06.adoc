...
By default, you brought into the user workload tab now this contains all of the workloads that are considered your workloads. If you click on the deployment tab, you'll see Apache struts back on Atlas cart service other applications that we deployed. You'll also see some that slipped through the cracks for for example, example CERT manager or cluster manager add-on these are currently services that were not added to ACS as part of the platform workloads now not to worry a simple command and API call can change this or or you can go to the platform configuration -> system configuration tab to change these details. For now, i've put together a quick script to move all of the services that are not part of the platform workloads into the user workload tab and vice versa

[start=1]
. You can use the following `curl` command to classify a workload (e.g., mark a deployment as a *user* workload):

[source,sh,role=execute]
----
curl -k -X PATCH \
  "$ROX_CENTRAL_ADDRESS/v1/deployments/bulkclassify" \
  -H "Authorization: Bearer $ROX_API_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
        "classification": "USER",               # or "PLATFORM"
        "deploymentIds": ["<deployment_id_1>", "<deployment_id_2>"]
      }'
----

- To get `deployment_id`, you can list deployments via the API:
[source,sh,role=execute]
----
curl -k -X GET "$ACS_CENTRAL/v1/deployments" \
  -H "Authorization: Bearer $ACS_API_TOKEN"
----
This will return a list of deployments and their IDs.

Replace the `classification` value with `USER` or `PLATFORM` as needed.
====

You can use the above method (or `roxctl deployment classify`) to reorganize workloads in your lab similarly to the steps seen in *11-configure-rhacs-settings*. This is particularly useful in large environments where workloads move between platform and user responsibility over time.
