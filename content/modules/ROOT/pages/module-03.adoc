...
We're going to use our patient portal frontend application to demonstrate the signing and verification process. First, we need to rebuild the application and push it to our local private registry. We sped up the process into two commands for you
...
[start=7]
. While you're in Quay, make the repo public so we can deploy the application without requiring credentials. Click on settings and then make the repository public.

image::12-quay-signed-image.png.png[link=self, window=blank, width=100%]
...
=== Re Deploy the Patient Portal Frontend Application 

Since our other skupper-patient-demo application is deployed to the patient-portal namespace let's quickly deploy it to the signed-patient-portal namespace.

. Run the following command in the terminal:

[source,sh,subs="attributes",role=execute]
----
oc create namespace signed-patient-portal
git clone https://github.com/mfosterrox/demo-applications.git demo-applications
cd demo-applications/skupper-demo
sed -i "s|quay.io/skupper/patient-portal-frontend:latest|$QUAY_URL/$QUAY_USER/patient-portal-frontend:1.0|g" frontend.yml
sed -i "s|namespace: patient-portal|namespace: signed-patient-portal|g" *.yml
grep -r "namespace: signed-patient-portal" *.yml
----
...
[source,sh,subs="attributes",role=execute]
----
oc apply -f database.yml -n signed-patient-portal
oc apply -f frontend.yml -n signed-patient-portal
oc apply -f payment.yml -n signed-patient-portal
----
...
[source,sh,subs="attributes",role=execute]
----
oc get pods -n signed-patient-portal
oc get route -n signed-patient-portal
----

*Let's take a look at it in RHACS.*

. Navigate to Vulnerability Management -> *Results* tab and click on the "Images" button

image::vuln-tssc-01.png[link=self, window=blank, width=100%]

use the drop downs to select for the signed-patient-portal namespace and the patient-portal-frontend image.

image::vuln-tssc-02.png[link=self, window=blank, width=100%]

You will see that the image is signed and the signature is valid. 

[start=4]
. Scan the signed image using roxctl to check for vulnerabilities:

[source,sh,subs="attributes",role=execute]
----
roxctl image scan --image $QUAY_URL/$QUAY_USER/patient-portal-frontend:1.0
----

[.console-output]
[source,bash,subs="+macros,+attributes"]
----
[lab-user@bastion ~]$ roxctl image scan --image $QUAY_URL/$QUAY_USER/
----

image::vuln-tssc-03.png[link=self, window=blank, width=100%]
