...
[TIP]
====
Should you see some extra characters in one of the tmux panes just delete them.
====
...
. Filter by the policy violation *Policy*, *Name*, `r/apk` OR by the most recent policy violations (sort by time). You will see a policy violation that has been enforced.
+
