...
. Click Next
. Select the two clusters: *aws-us* and *production* (The production cluster is the local-cluster in the lab)
. Click Next
...
Great job updating your allowed image registries! This is one of the fastest ways to improve security and compliance in OpenShift.

Other quick hardening steps include enabling audit log forwarding and activating host node auditing with Linux auditd. These settings can be applied cluster-wide using the MachineConfig Operator. Even better, you can bake them into new clusters automatically from the start using tools like Ansible, Advanced Cluster Management (ACM), or GitOps workflows.

When Security and Operations teams work closely together, you can provision clusters with strong baseline controls right from day one and keep them consistently secure across the environment.
...
Package managers such as apt, apk, yum, and dnf are useful on virtual machines, but they have no place in production containers.

Allowing them enables uncontrolled software installation, dramatically increases the attack surface, and breaks auditability and immutability — all of which are core requirements for sovereign cloud environments.

We will create a simple RHACS runtime policy that monitors running processes using Linux kernel instrumentation, detects attempts to execute package managers, and automatically terminates violating pods by leveraging OpenShift enforcement.

This single policy delivers powerful outcomes. It prevents sneaky software installs, enforces immutability, reduces overall risk, and maintains strong sovereign control over what executes in your clusters.
