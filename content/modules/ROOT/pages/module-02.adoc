...
IMPORTANT: The RHACS compliance coverage dashboard provides a comprehensive overview of your scan schedules and results. In the Coverage tab, you can review all scan schedules or filter to view specific schedules such as the "acs-catch-all". Each scan displays coverage across a variety of compliance standards—including CIS, OCP, E8, NERC-CIP, NIST 853, PCI-DSS, and DISA-STIG. You can filter results by specific checks for greater detail. On the right side of the dashboard, you’ll see the cluster’s overall compliance status, along with a breakdown of which checks are passing or failing.
...
[start=1]
. First, verify the pods are running in the patient-portal namespace:
...
[start=5]
. OpenShift enforces strict security policies by default (via the restricted Security Context Constraint or SCC). Now, create a security policy to enforce runtime protection against package manager execution. Apply the following policy configuration:
...
. In RHACS, navigate to the Violations page.
...
IMPORTANT: You must resolve the issue before it will disappear from the Violations page.

[start=4]
. Remove the deployment that violates the policy by click the three dots and selecting "Mark as resolved"
...
If you love vulnerability tracking you can read more about it here: https://access.redhat.com/security/updates/classification

But we wanted to show you that all of Red Hat's software is signed, attested, and reaching compliance with SLSA Level 3 standards (see https://slsa.dev/spec/v1.0/levels#build-l3-hardened-builds[SLSA Level 3 compliance details]).
...
For more information, visit the official Red Hat Project Hummingbird page: https://www.redhat.com/en/technologies/linux-platforms/enterprise-linux/hummingbird
