...
[start=5]
. Exit from the container (type exit)and return to the bastion host terminal.
...
.. Type `exit` to exit the tmux session and return to the bastion host terminal.
...
If you love vulnerability tracking you can read more about it here - https://access.redhat.com/security/updates/classification. But we wanted to show you that all of Red Hat's software is signed, attested, and reaching compliance with SLSA Level 3 standards (see https://slsa.dev/spec/v1.0/levels#build-l3-hardened-builds[SLSA Level 3 compliance details]).
